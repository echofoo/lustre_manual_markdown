...
| setstripe_options                | Specifies a specific layout for the mirror. It can be a plain layout with a specific striping pattern or a composite layout, such as [the section called “Progressive File Layout(PFL)”](03.8-Managing%20File%20Layout%20(Striping)%20and%20Free%20Space.md#progressive-file-layoutpfl). The options are the same as those for the `lfs setstripe` command.If *setstripe_options* are not specified, then the stripe options inherited from the previous component will be used. If there is no previous component, then the `stripe_count` and`stripe_size` options inherited from the filesystem-wide default values will be used, and the OST `pool_name` inherited from the parent directory will be used. |
